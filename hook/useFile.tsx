...
const useFile = (folderID: string) => {
  ...
  const { data, mutate, error, isLoading } = useSWR<IFileFetch, Error>(
    `/folder/${folderID}`,
    FecherFile,
    {
  ...
  useEffect(() => {
    if (data) {
      const rootFolder = {
        ...
        children: [
          ...data.child.map((item) => item._id),
          ...data.files.map((item) => item._id),
        ],
        shared: data.shared,
      } as Item;
      ...
      if (folderID) setChildren([rootFolder, ...allFolder, ...allFile]);
    }
