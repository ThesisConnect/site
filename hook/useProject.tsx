...
import {
  checkSchemaCreateProject,
  createNewProjectType,
} from '../models/Project/createNewProject';
...
interface updateProjectType
  extends Omit<
    editSchema,
    'status' | 'progress' | 'advisors' | 'co_advisors' | 'advisee'
  > {
  advisors: string[];
  co_advisors: string[];
  advisee: string[];
...
const useProject = (suspense: boolean = false): IProjectHookReturn => {
  ...
  const { data, error, mutate, isLoading } = useSWR<IProject[], Error>(
    '/test/page/main',
    fetcher,
    { revalidateOnFocus: true }
  );
  const updateProject = useCallback(
    async (updateData: updateProjectType, projectID: string) => {
      try {
  ...
  const createNewProject = useCallback(
    async (createData: createNewProjectType) => {
      try {
        const newData = {
  ...
  const deleteProject = useCallback(
    async (projectID: string) => {
      try {
        const newDataWithDelete = project.filter(
