...
const usePlanByProjectID = (ProjectID: string): PlanHookReturn => {
  const { getPlanByProjectID, setPlanEachProject } = useProjectStore(
    (state) => ({
      getPlanByProjectID: state.getPlanByProjectID,
      setPlanEachProject: state.setPlanEachProject,
    })
  );
  const { data, error, mutate, isLoading } = useSWR<IPlan[], Error>(
    `/page/plan/${ProjectID}`,
    fetcher,
    {
      onSuccess: (data) => {
        if (ProjectID) setPlanEachProject(ProjectID, data);
      },
    }
  );
