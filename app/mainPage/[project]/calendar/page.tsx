...
function PageCalendar({ params: { project } }: { params: { project: string } }) {
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-hidden">
      {state && (
        selectItem.map((obj) => (
          <DetailPopup show={state} key={v4()}
          id={obj._id} name={obj.name} description={obj.description} start_date={GetFormatDate(obj.start_date)} end_date={GetFormatDate(obj.end_date)} progress={obj.progress} task={obj.task} duration={getDayDiff(obj.start_date, obj.end_date)} onClose={showPlanDetail} />
        ))
      )}
