...
const DeleteFileAndFolderPopup: React.FC<ModalProps> = ({
  ...
  const onDelete = async () => {
    setIsLoading(true);
    try {
      const resData = await axiosBaseurl.delete(`/file/delete/${dataFileOrFolder._id}`,{
        params: {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full flex justify-center z-50 flex-col items-center bg-black bg-opacity-50">
      <div className={"bg-white rounded-t-lg overflow-y-scroll p-3 w-96 min-h-[10rem] max-h-80 flex flex-col  justify-evenly items-center"+  (isLoading ? " rounded-lg" : "")}>
        ...
        {isLoading ? (
          ...
          <div className="flex gap-1 flex-col">
            ...
            <p className="text-center text-sm font-semibold">
              ...
              ${dataFileOrFolder.name} and all its content?` : dataFileOrFolder.name+" ?"}{" "}
            </p>
          </div>
        )}
      </div>
